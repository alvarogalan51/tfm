function [status varargout] = lucasdigicamident(opMode, inpath, outpath, prnuName, iList, addargs)
%
% Function: [status varargout] = lucasdigicamident(opMode, inpath, outpath, iList, addargs)
%
...
case 'reference'        % Generate reference images
    ...
    [status1, denoiseFolder] = denoiseImages(inpath, iList, addargs);
    ...
    for iFile = 1:Np
        ...
        thisSize = size (denIm);
        if (thisSize(1) ~= prevSize(1))
            srcIm = imrotate(srcIm, 90);
            denIm = imrotate(denIm, 90);
        end
    ...
    diffImageName = fullfile(denoiseFolder, [prnuName{1} '.png']);
